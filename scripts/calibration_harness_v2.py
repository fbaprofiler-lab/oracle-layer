#!/usr/bin/env python3
"""
Calibration Harness v2 - Run Jev predictions on closed markets by category
...
EVAL_SPEC = {
    ...
    "description": "Calibration evaluation v2 - Jev on closed Polymarket markets by target category",
    "markets_source": "gamma-api.polymarket.com closed markets with outcomePrices, filtered by category",
    "target_categories": ["fed-rate-decisions", "cpi-inflation", "crypto-prices", "geopolitics", "election-politics"],
    "target_n": 100,
    "gates": {
        "ece_threshold": 0.05,
...
def run_evaluation() -> Dict:
    ...
    print(f"\n{'='*60}")
    print(f"ORACLE LAYER - CALIBRATION EVALUATION v2")
    ...
    if len(unique_resolved) < EVAL_SPEC['target_n']:
        print(f"WARNING: Only {len(unique_resolved)} markets available, target was {EVAL_SPEC['target_n']}")
    ...
    for i, market in enumerate(test_markets, 1):
        ...
        try:
            ...
            results.append({
                "question": question, "condition_id": condition_id, "category": category,
                "end_date": end_date, "actual_outcome": actual_outcome,
                "actual_price": actual_yes_price, "latest_market_price": latest_price,
                "jev_prob": round(jev_prob, 4), "jev_confidence": round(jev_conf, 4),
                "correct": correct, "raw_jev": jev_result['raw_response'],
            })
            ...
            results.append({
                ...
                "end_date": end_date, "actual_outcome": actual_outcome,
                "actual_price": actual_yes_price, "latest_market_price": latest_price,
                "jev_prob": None, "jev_confidence": None, "correct": None, "error": str(e),
            })
    ...
    report = {
        "evaluation_spec": EVAL_SPEC, "results": results, "metrics": metrics,
        "gates": {
